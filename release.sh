SCREEN_RECORDER_VERSION=`grep '<screen-recorder.version>' pom.xml | sed -r 's#.*>([^<]+)<.*#\1#'`

echo "Project defines following version of ScreenRecorder: ${SCREEN_RECORDER_VERSION}"
echo $SCREEN_RECORDER_VERSION | egrep -q '\-SNAPSHOT$' && { echo "The project cannot depend on SNAPSHOT version, correct the version first and try again."; exit 2; }
